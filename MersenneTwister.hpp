...
//! \return integer between 0 and max
  ...
  vector<int> randomValues(int minValue, int maxValue, unsigned int numValues, bool replacement = true) {
    if (numValues < 0)
      throw invalid_argument("Number of values must be a positive integer");
    if (minValue>maxValue)
      throw invalid_argument("Minimum value larger than maximum value");

    vector<int> myvector(numValues);
    if (replacement)
      for (int &i : myvector)
